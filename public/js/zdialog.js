...
;(function($, window, document, undefined) {
  ...
  function loadBox(Url) {
    var url = Url
    var dislogContainer = $(
      '<div class="zbox-popup" style="display: block;"><img  id="zchange" src="' +
        url +
        '"></div>'
    )
    var blackFilter = $(
      '<div class="zbox-popup-backdrop" style="display: block;"></div>'
    )
    setTimeout(function() {
  ...
  function Show(params, caller) {
    ...
    if (caller == 'Autofade') {
      console.log(PARAMS)
      var typeClass
      if (PARAMS.Type === 'success') typeClass = 'primary'
      else if (PARAMS.Type === 'error') typeClass = 'error'
      dislogContainer = $(
        '<div class="zbox-toast-container ' +
          typeClass +
          '"><div class="zbox-toast-message">' +
          PARAMS.Content +
          '</div></div>'
      )
      $('body').append(dislogContainer)
      ...
      dislogContainer = $(
        '<div class="zbox-popup" style="display: block;"></div>'
      )
      dialogInner = $(
        '<div class="zbox-popup-inner"><div class="zbox-popup-title">' +
          PARAMS.Title +
          '</div><div class="zbox-popup-text">' +
          PARAMS.Content +
          '</div></div>'
      )
      dialogBtn = $(
        '<div class="zbox-popup-buttons"><span class="zbox-popup-button" index="0">' +
          PARAMS.BtnL +
          '</span></div>'
      )
      if (caller == 'Confirm') {
        dialogBtn.append(
          $(
            '<span class="zbox-popup-button R" index="1">' +
              PARAMS.BtnR +
              '</span>'
          )
        )
